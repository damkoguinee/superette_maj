...
class FacturationController extends AbstractController
{
    ...
    public function validation(LieuxVentes $lieu_vente, Request $request, FacturationRepository $facturationRep, StockRepository $stockRep, ListeStockRepository $listeStockRep, CaisseRepository $caisseRep, DeviseRepository $deviseRep, FraisSupRepository $fraisSupRep, CategorieOperationRepository $categorieOpRep, CompteOperationRepository $compteOpRep,  SessionInterface $session, ModePaiementRepository $modePaieRep, UserRepository $userRep, ProductsRepository $productRep, LiaisonProduitRepository $liaisonRep, ProformatRepository $proformatRep, PromotionRepository $promotionRep, EntityManagerInterface $em): Response
    {
        ...
        if ($panier) {
            ...
            $bonusAdded = false; // Variable pour vérifier si le bonus a déjà été ajouté
            foreach ($panier as $item) {
                ...
                foreach ($promotions as $promotion) {
                    ...
                    if ($totalQuantity >= $promotion->getQuantiteMin()) {
                        ...
                        if ($factor >= 4 && $factor < 8) {
                            $quantiteOfferte = ($factor * $promotion->getQuantiteBonus()) + $promotion->getQuantiteBonus();
                        } elseif ($factor == 8) {
                            $quantiteOfferte = ($factor * $promotion->getQuantiteBonus()) + 2 * $promotion->getQuantiteBonus();
                        } elseif ($factor > 8 && $factor <= 9) {
                            $quantiteOfferte = ($factor * $promotion->getQuantiteBonus()) + 2 * $promotion->getQuantiteBonus() + 1;
                        } elseif ($factor > 9 && $factor <= 10) {
                            $quantiteOfferte = ($factor * $promotion->getQuantiteBonus()) + 2 * $promotion->getQuantiteBonus() + 1.5 ;
                        } elseif ($factor > 10 && $factor <= 11) {
                            $quantiteOfferte = ($factor * $promotion->getQuantiteBonus()) + 2 * $promotion->getQuantiteBonus() + 2;
                        } elseif ($factor > 11) {
                            $quantiteOfferte = ($factor * $promotion->getQuantiteBonus()) + 2 * $promotion->getQuantiteBonus() + 2.5;
                        } else {
                            $quantiteOfferte = $factor * $promotion->getQuantiteBonus();
                        }

                        // Trouver le produit du panier avec la plus grande quantité et le plus grand stock disponible
                        $maxQuantityItem = null;
                        foreach ($panier as $cartItem) {
                            if (!$maxQuantityItem) {
                                $maxQuantityItem = $cartItem;
                            } else {
                                // Comparer les quantités dans le panier
                                if ($cartItem['qtite'] > $maxQuantityItem['qtite']) {
                                    $maxQuantityItem = $cartItem;
                                } elseif ($cartItem['qtite'] === $maxQuantityItem['qtite']) {
                                    // Si les quantités sont égales, comparer les stocks disponibles
                                    if ($cartItem['dispo'] > $maxQuantityItem['dispo']) {
                                        $maxQuantityItem = $cartItem;
                                    }
                                }
                            }
                        ...
                        // Ajouter le produit bonus au panier
                        if ($maxQuantityItem && !$bonusAdded) {
                            $panier[] = [
                            ...
                            $bonusAdded = true; // Marquer que le bonus a été ajouté
                            break; // Sortir de la boucle de promotions
                        }
                    }
                }

                if ($bonusAdded) {
                    break; // Sortir de la boucle principale si le bonus a été ajouté
                }
            }
