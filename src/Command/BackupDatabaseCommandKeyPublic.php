...
class BackupDatabaseCommandKeyPublic extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        // Utilisation de la clé publique SSH configurée
        $sshKeyPath = 'C:/wamp64/projets_symfony/ssh/id_rsa';

        $scpCommand = [
            'scp',
            '-i', $sshKeyPath, // Spécifie la clé privée pour l'authentification
            '-P', $sshPort,
            $cheminDump,
        ...
        $importProcess = new Process(['bash', '-c', $importCommand]);
        $importProcess->run();

        if (!$importProcess->isSuccessful()) {
            $io->error('Échec de l\'importation de la base de données sur le serveur distant : ' . $importProcess->getErrorOutput());
            return Command::FAILURE;
        }
        ...
        $renameProcess = new Process(['ssh', '-i', $sshKeyPath, '-p', $sshPort, $sshUser . '@' . $sshHost, $renameCommand]);
        $renameProcess->run();
